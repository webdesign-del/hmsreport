...
function adherencePct(act: number, exp: number): number {
  return exp > 0 ? Math.round((act / exp) * 100) : 0;
}

export default function KpiSubtabs({ kpi }: { kpi: DashboardSummaryResponse["kpi"] }) {
    ...
    <div>
      ...
      {tab === 0 && (
        <KpiRow>
          ...
          <Kpi icon="◑" color="blue" label="Adherence" value={`${adherencePct(kpi.today.act, kpi.today.exp)}%`} sub="actual ÷ expected" />
      ...
      {tab === 1 && (
        <KpiRow>
          ...
          <Kpi icon="◑" color="blue" label="Adherence" value={`${adherencePct(kpi.week.act, kpi.week.exp)}%`} sub="actual ÷ expected" />
      ...
      {tab === 2 && (
        <KpiRow>
          ...
          <Kpi icon="◑" color="blue" label="Adherence" value={`${adherencePct(kpi.month.act, kpi.month.exp)}%`} sub="actual ÷ expected" />
